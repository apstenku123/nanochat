...
def verify_cpp_tokenizer(tokenizer, sample_code=None):
    ...
    if sample_code is None:
        ...
        sample_code = (
            "class MyClass { public: virtual void process() const override; };"
        )

    tokenizer.encode(sample_code)
    ...
    if not is_cpp_optimized:
        print(
            f"WARNING: C++ tokenizer check: only {keywords_single}/{keywords_found} C++ keywords "
            f"are single tokens. This tokenizer may not be optimal for C++ code."
        )
        print("NOTE: NANOCHAT_CPP_TOKENIZER=1 is now the default.")
        print(
            "      Set NANOCHAT_CPP_TOKENIZER=0 to use RustBPE tokenizer for non-C++ training."
        )
...
def _compute_token_bytes_from_tokenizer(tokenizer):
    """Compute token_bytes tensor directly from tokenizer (no file needed).

    Returns 1D int32 tensor of shape (vocab_size,) where each entry is the
    UTF-8 byte length of that token, or 0 for special/unknown tokens.

    Uses the underlying HuggingFace tokenizer's decode when available, because
    byte-level BPE tokenizers store vocab keys in byte-level encoding (e.g.
    'Ġhello' for ' hello') whose UTF-8 byte count differs from the actual
    token's byte count. The HF decode path correctly reverses byte-level encoding.
    """
    import torch
    ...
    # For CppTokenizer / HuggingFaceTokenizer, use underlying HF tokenizer
    # which correctly handles byte-level BPE decoding
    hf_tok = getattr(tokenizer, "_tokenizer", None) or getattr(tokenizer, "tokenizer", None)

    for tid in range(vocab_size):
        try:
            if hf_tok is not None:
                decoded = hf_tok.decode([tid], skip_special_tokens=False)
            else:
        ...
        if not decoded:
            continue
        # Special tokens decode to their tag strings (e.g. "<|bos|>") — skip them
        if decoded.startswith("<") and decoded.endswith(">") and len(decoded) > 2:
            continue
        token_bytes[tid] = len(decoded.encode("utf-8"))
    return token_bytes


def get_token_bytes(device="cpu", tokenizer=None):
    """Get token_bytes tensor for BPB evaluation.

    If tokenizer is provided, computes token_bytes directly from it (always
    matches vocab_size exactly). Otherwise falls back to loading token_bytes.pt.
    """
    import torch

    if tokenizer is not None:
        token_bytes = _compute_token_bytes_from_tokenizer(tokenizer)
        if str(device) != "cpu":
