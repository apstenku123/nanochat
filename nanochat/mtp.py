...
class MTPModule(nn.Module):
    ...
    def forward(self, hidden_states, next_token_ids, wte, lm_head_weight,
        ...
        h = hidden_states[:, :-1]  # (B, T-1, C)
        next_emb = wte(next_token_ids[:, :-1])  # (B, T-1, C)

        # RMSNorm both inputs independently, then concatenate and project
        h_norm = F.rms_norm(h, (C,))
        e_norm = F.rms_norm(next_emb, (C,))
        combined = torch.cat([h_norm, e_norm], dim=-1)  # (B, T-1, 2C)
        h_mtp = self.proj(combined)  # (B, T-1, C)
        ...
        # MTP targets: tokens at positions 2..T = next_token_ids[:, 1:]
        mtp_targets = next_token_ids[:, 1:].contiguous()  # (B, T-1)
