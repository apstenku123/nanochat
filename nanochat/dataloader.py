...
def _document_batches(
    split, resume_state_dict, tokenizer_batch_size, data_dir=None, streaming=False
):
    """
    ...
    if streaming:
        ...
        while True:
            ...
            if parquet_paths:
                print(
                    f"[streaming] Found {len(parquet_paths)} {split} shards, starting training"
                )
                break
    ...
    while True:  # iterate infinitely (multi-epoch)
        ...
        if streaming:
            ...
            if len(new_paths) > known_count:
                print(
                    f"[streaming] Discovered {len(new_paths) - known_count} new shards ({len(new_paths)} total)"
                )
...
def tokenizing_distributed_data_loader_with_state(
    ...
    fim_rate=0.0,
    spm_rate=0.5,
    structured_fim_rate=0.0,
    function_fim_rate=0.0,
    structured_fim_path=None,
    data_dir=None,
    streaming=False,
):
    """
    ...
    # FIM configuration
    use_fim = (
        fim_rate > 0 or structured_fim_rate > 0 or function_fim_rate > 0
    ) and split == "train"
    structured_dataset = (
        get_structured_fim_dataset(tokenizer, structured_fim_path)
    ...
    def refill_buffer():
        ...
        if use_fim:
            ...
            if structured_fim_rate > 0 or function_fim_rate > 0:
                # Mixed FIM: random + structured + function-level
                ...
                # Random FIM only
                content_lists = apply_fim_batch(
                    content_lists, fim_rate=fim_rate, spm_rate=spm_rate
                )
...
def binary_distributed_data_loader_with_state(
    B, T, split, device="cuda", resume_state_dict=None
):
    """
    ...
    assert len(bin_files) > 0, (
        "No tokenized .bin files found. Run: python -m scripts.pretokenize"
    )
    ...
    if len(bin_files) == 1:
        if split == "train":
            raise ValueError(
                "Cannot create train split: only 1 .bin file found, but need at least 2 "
                "(last file is used for validation). Either add more data shards or use split='val'."
            )
    else:
    ...
    tokens_per_batch = (
        B * T * ddp_world_size
    )  # tokens consumed per iteration across all ranks
...
def binary_distributed_data_loader(*args, **kwargs):
    ...
    for inputs, targets, state_dict in binary_distributed_data_loader_with_state(
        *args, **kwargs
    ):
        yield inputs, targets
