...
All datasets are **COMPLETE** with `_COMPLETE` sentinels, converted to parquet, and uploaded to GCS.

#### Legacy JSONL Datasets (direct chunker output)
...
| `cpp_compilable_16k_v2` | 1.0M | 7.3 GB | 16K | Compilable: types→functions |
| `cpp_compilable_64k_v2` | 394K | 3.3 GB | 64K | Compilable: types→functions |

#### Parquet Datasets (`gs://nanochat-training-data-2026/data/parquet/`)

Generated by `scripts/data/build_all_chunk_sizes.sh` on build3. Three methods across 5 token sizes.

**Tree-sitter Compilable** (93 OSS projects + GPU/NPU repos, compilable ordering):

| Dataset | Docs | Shards | Size | Status |
|---------|------|--------|------|--------|
| `treesitter_compilable_16k` | 1.64M | 33 train + 2 | — | COMPLETE |
| `treesitter_compilable_64k` | 666K | 33 train + 2 | — | COMPLETE |
| `treesitter_compilable_128k` | 71.7K | 8 train + 2 | 12.4 GB | COMPLETE |
| `treesitter_compilable_256k` | 36.0K | 8 train + 2 | 12.4 GB | COMPLETE |
| **`treesitter_compilable_512k`** | **18.6K** | **10 train + 2** | **12.3 GB** | **COMPLETE** |

> 128K/256K/512K are the same source data concatenated into fewer, larger documents
> via `scripts/data/concat_compilable_docs.py` (groups by project, fills to target size).
> Total bytes are similar (~12.3 GB) because it's the same content, just packed differently.

**Git Commit Chains** (pre→post code from git history):

| Dataset | Docs | Shards | Size | Status |
|---------|------|--------|------|--------|
| `git_commit_chains_16k` | — | — | — | COMPLETE |
| `git_commit_chains_128k` | 4.89M | 485 train + 2 | — | COMPLETE |
| `git_commit_chains_256k` | 4.89M | 969 train + 2 | — | COMPLETE |
| **`git_commit_chains_512k`** | **4.89M** | **2,423 train + 2** | **23.2 GB** | **COMPLETE** |

> Sources: commitpack (2M records), git_history (502K), gpu_npu (445K).
> Doc count is stable across sizes because most commit diffs are <128K tokens.

**Other Parquet Datasets**:

| Dataset | Description | Status |
|---------|-------------|--------|
| `clang_semantic_16k` | Clang-indexed cross-file deps | COMPLETE |
| `gpu_npu_compilable_16k` | GPU/NPU project compilable units | COMPLETE |
| `gpu_npu_compilable_64k` | GPU/NPU project compilable units | COMPLETE |
| `ms_src_cpp` | Microsoft C++ sources | COMPLETE |
| `commitpack_chains` | CommitPack raw chains | COMPLETE |
| `git_history_chains` | Git history raw chains | COMPLETE |
| `gpu_npu_chains` | GPU/NPU commit chains | COMPLETE |

### Data Pipeline Scripts

| Script | Purpose |
|--------|---------|
| `scripts/data/build_all_chunk_sizes.sh` | **Master orchestrator**: runs cpp-chunker for 3 methods × 5 sizes, converts to parquet |
| `scripts/data/concat_compilable_docs.py` | Concatenates small docs into 128K/256K/512K by project grouping |
| `scripts/data/batch_jsonl_to_parquet.py` | JSONL → shuffled parquet shards with val split and `_COMPLETE` sentinel |
| `scripts/data/stream_jsonl_to_parquet.py` | Real-time streaming JSONL → parquet (watches growing file) |
| `scripts/stream_upload_parquet.sh` | Watches local dir, uploads parquet shards to GCS as created |
| `scripts/stream_download_parquet.sh` | Watches GCS, downloads shards as they appear |

### Training with Compilable Data

```bash
# Download from GCS (any dataset)
gcloud storage cp -r gs://nanochat-training-data-2026/data/parquet/treesitter_compilable_64k/ /path/to/data/

# Train (GPU)
python -m scripts.base_train --data_dir=/path/to/data/treesitter_compilable_64k \
...
### Long-Context Concatenation (128K–512K)

For context lengths beyond 64K, individual compilable documents are too small.
`scripts/data/concat_compilable_docs.py` solves this by:

1. Grouping documents by project (extracted from preamble)
2. Concatenating related compilation units within each project
3. Filling to target token limit with separator comments between units
4. Enforcing min 25% fill (drops tiny leftover groups)

```bash
# Generate 512K concatenated docs from 64K source
python3 scripts/data/concat_compilable_docs.py \
    --input treesitter_compilable_64k.jsonl \
    --output treesitter_compilable_512k.jsonl \
    --max_tokens 524288
```
