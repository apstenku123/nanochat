...
parser.add_argument(
    "--head_dim", type=int, default=128, help="target head dimension for attention"
)
parser.add_argument(
    "--max_seq_len",
    type=int,
    default=2048,
    help="max context length (supports up to 16384 for long context)",
)
parser.add_argument(
    "--window_pattern",
    type=str,
...
parser.add_argument(
    "--engram", action="store_true", help="enable optional Engram branches"
)
parser.add_argument(
    "--engram_layers",
    type=str,
    default="",
    help="comma-separated layer indices for Engram insertion (empty=disabled)",
)
parser.add_argument(
    "--engram_ngram_orders",
    type=str,
    default="2,3,4",
    help="comma-separated n-gram orders for Engram branch",
)
parser.add_argument(
    "--engram_bottleneck_dim",
    type=int,
    default=0,
    help="Engram bottleneck dimension (0=auto)",
)
parser.add_argument(
    "--engram_dropout", type=float, default=0.0, help="dropout on Engram branch"
)
parser.add_argument(
    "--mhc", action="store_true", help="enable optional mHC branch mixing"
)
parser.add_argument(
    "--mhc_num_branches", type=int, default=0, help="mHC branch count (0=auto)"
)
parser.add_argument(
    "--mhc_sinkhorn_iters", type=int, default=5, help="mHC Sinkhorn iterations"
)
parser.add_argument(
    "--mhc_temperature", type=float, default=1.0, help="mHC transport temperature"
)
parser.add_argument(
    "--mhc_epsilon", type=float, default=1e-6, help="mHC numerical epsilon"
)
parser.add_argument(
    "--mhc_blend_alpha", type=float, default=1.0, help="global mHC blend strength"
)
parser.add_argument(
    "--aux_loss_weight",
    type=float,
    default=0.0,
    help="auxiliary regularization loss weight",
)
# Multi-Token Prediction (DeepSeek-V3 style)
parser.add_argument(
    "--mtp",
    action="store_true",
    help="enable Multi-Token Prediction (predicts token i+2)",
)
parser.add_argument(
    "--mtp_lambda",
    type=float,
    default=0.3,
    help="MTP loss weight (DeepSeek uses 0.3 early, 0.1 later)",
)
# DeepSeek Sparse Attention (DSA)
parser.add_argument(
    "--dsa",
    action="store_true",
    help="enable DeepSeek Sparse Attention from dsa_start_layer to last layer",
)
parser.add_argument(
    "--dsa_start_layer",
    type=int,
    default=7,
    help="first layer to use sparse attention (0-indexed)",
)
parser.add_argument(
    "--dsa_top_k_ratio",
    type=float,
    default=0.5,
    help="fraction of tokens to attend to in sparse layers",
)
parser.add_argument(
    "--dsa_local_window",
    type=int,
    default=128,
    help="local window always included in sparse attention",
)
parser.add_argument(
    "--dsa_indexer_heads",
    type=int,
    default=16,
    help="number of lightweight indexer heads for DSA",
)
parser.add_argument(
    "--dsa_indexer_dim", type=int, default=32, help="dimension per indexer head for DSA"
)
# Mamba-2/3 hybrid layers
parser.add_argument(
    "--mamba",
    action="store_true",
    help="enable Mamba-2 hybrid layers (pattern controls which layers)",
)
parser.add_argument(
    "--mamba_pattern",
    type=str,
    default="AAM",
    help="layer pattern: A=attention, M=mamba, tiled across layers (e.g. AAM, AM, M)",
)
parser.add_argument("--mamba_d_state", type=int, default=64, help="SSM state dimension")
parser.add_argument(
    "--mamba_d_conv", type=int, default=4, help="depthwise conv kernel width"
)
parser.add_argument(
    "--mamba_expand",
    type=int,
    default=2,
    help="expansion factor (d_inner = expand * n_embd)",
)
parser.add_argument(
    "--mamba_headdim", type=int, default=128, help="head dimension for SSD"
)
parser.add_argument(
    "--mamba_ngroups", type=int, default=1, help="number of groups for B/C (GQA-like)"
)
parser.add_argument(
    "--mamba_chunk_size", type=int, default=256, help="chunk size for SSD scan"
)
parser.add_argument(
    "--mamba3_qknorm", action="store_true", help="Mamba-3 Phase 2: QK-norm on B/C"
)
parser.add_argument(
    "--mamba3_bias", action="store_true", help="Mamba-3 Phase 2: learnable B/C bias"
)
parser.add_argument(
    "--mamba3_complex_rope",
    action="store_true",
    help="Mamba-3 Phase 2: complex RoPE on B/C",
)
parser.add_argument(
    "--mamba3_trapezoidal",
    action="store_true",
    help="Mamba-3 Phase 3: trapezoidal discretization",
)
parser.add_argument(
    "--mamba_xla_scan",
    action="store_true",
    help="use XLA scan backend for Mamba on TPU",
)
parser.add_argument(
    "--rope_theta",
    type=float,
    default=10000.0,
    help="RoPE base theta for attention and Mamba complex RoPE",
)
parser.add_argument(
    "--window_long",
    type=int,
    default=0,
    help="long attention window override (0 -> sequence_len)",
)
parser.add_argument(
    "--window_short",
    type=int,
    default=0,
    help="short attention window override (0 -> window_long//2)",
)
# Memory optimization
parser.add_argument(
    "--gradient_checkpointing",
    action="store_true",
    help="enable gradient checkpointing (saves memory, trades compute)",
)
parser.add_argument(
    "--tensor_parallel",
    type=int,
    default=1,
    help="tensor parallelism degree (1=data-only, 2/4/8=split model across chips)",
)
...
parser.add_argument(
    "--adam_beta1", type=float, default=0.8, help="Adam beta1 for embedding/unembedding"
)
parser.add_argument(
    "--adam_beta2",
    type=float,
...
parser.add_argument(
    ...
    type=int,
    default=-1,
    help="resume training from this step (-1 = disable, -2 = auto-detect latest checkpoint)",
)
# Evaluation
parser.add_argument(
    "--eval_every",
...
parser.add_argument(
    "--tokenizer_batch_size", type=int, default=128, help="batch size for tokenization"
)
parser.add_argument(
    "--fim_rate",
    type=float,
    default=0.0,
    help="Fill-in-the-Middle rate (0.0=disabled, 0.5=50%% of docs get FIM). Requires NANOCHAT_CPP_TOKENIZER=1",
)
parser.add_argument(
    "--structured_fim_rate",
    type=float,
...
parser.add_argument(
    "--precision", type=str, default="auto", help="precision: auto|nvfp4|fp8|bf16"
)
parser.add_argument(
    "--nvfp4_disable_rht",
    type=bool,
    default=True,
    help="disable Random Hadamard Transform (required for SM121/GB10)",
)
parser.add_argument(
    "--nvfp4_disable_sr",
    type=bool,
    default=True,
    help="disable Stochastic Rounding (required for SM121/GB10)",
)
# FP8 training with torchao (separate from TE-based --precision fp8)
parser.add_argument(
    "--fp8",
    action="store_true",
    help="enable FP8 training with torchao (requires H100+ GPU)",
)
parser.add_argument(
    "--fp8_recipe",
    type=str,
    ...
    help="FP8 scaling recipe: tensorwise (faster) or rowwise (more accurate)",
)
# Kernel backend
parser.add_argument(
    "--kernel",
    ...
    help="kernel backend: current (PyTorch), liger (Liger-Kernel), cce (Apple Cut Cross Entropy), triton (Unsloth-style)",
)
# torch.compile
parser.add_argument(
    "--no_compile",
    action="store_true",
    help="disable torch.compile (use for NVIDIA containers with triton issues)",
)
# XLA/TPU optimizations
parser.add_argument(
    "--use_scan",
    action="store_true",
    help="use torch_xla scan_layers to reduce XLA compilation time (TPU only)",
)
parser.add_argument(
    "--xla_flash_attn",
    action="store_true",
    help="use XLA Pallas flash attention for TPU (O(n) memory, enables long seq_len). Does not support sliding window - use --window_pattern=L",
)
parser.add_argument(
    "--chunked_attn",
    action="store_true",
    help="use chunked attention for long sequences on XLA/TPU. Pure PyTorch, no JAX needed. Reduces O(n^2) to O(n*chunk)",
)
parser.add_argument(
    "--attn_chunk_size",
    type=int,
    default=1024,
    help="chunk size for chunked attention (default: 1024)",
)
# Data directory
parser.add_argument(
    "--data_dir",
    type=str,
    default="",
    help="Custom parquet data directory (default: base_data from NANOCHAT_BASE_DIR)",
)
parser.add_argument(
    "--streaming_data",
    action="store_true",
    help="Streaming mode: dynamically discover new parquet shards as they arrive. Waits for _COMPLETE sentinel.",
)
...
def _build_gpt_config(model_config_kwargs):
    ...
    filtered_kwargs = {
        k: v for k, v in model_config_kwargs.items() if k in supported_keys
    }
    dropped_keys = sorted(set(model_config_kwargs.keys()) - set(filtered_kwargs.keys()))
    if dropped_keys:
        print0(
            f"Ignoring unsupported GPTConfig keys in this checkout: {', '.join(dropped_keys)}"
        )
...
def _apply_tensor_parallel_sharding(model, mesh):
    ...
    print0(
        f"Tensor parallelism: sharded {n_sharded} weight matrices across 'model' axis"
    )
...
def train():
    ...
    if device_type == "xla":
        ...
        if num_devices > 1:
            ...
            if tp_degree > 1:
                assert num_devices % tp_degree == 0, (
                    f"num_devices={num_devices} not divisible by tp={tp_degree}"
                )
                ...
                print0(
                    f"SPMD 2D mesh: {dp_degree}-way data × {tp_degree}-way tensor parallelism, {num_devices} TPU devices"
                )
            else:
                spmd_mesh = Mesh(device_ids, (num_devices,), ("data",))
                ddp_world_size = num_devices
                print0(
                    f"SPMD data parallelism: {num_devices} TPU devices, mesh=({num_devices},)"
                )
    ...
    precision_plan = select_precision(
        ...
        disable_sr=args.nvfp4_disable_sr,
    )
    ...
    if "NVFP4" in precision_plan.name and args.device_batch_size < 2:
        print0(
            f"WARNING: NVFP4 requires device_batch_size >= 2, but got {args.device_batch_size}"
        )
    ...
    if args.dsa:
        assert args.dsa_start_layer < num_layers, (
            f"dsa_start_layer ({args.dsa_start_layer}) must be < num_layers ({num_layers})"
        )
        dsa_layers = num_layers - args.dsa_start_layer
        print0(
            f"DSA enabled: layers {args.dsa_start_layer}-{num_layers - 1} ({dsa_layers} sparse layers, top_k_ratio={args.dsa_top_k_ratio})"
        )
    if args.mtp:
    ...
    model.to(
        device
    )  # Move model to GPU before init_weights (rotary embeddings need correct device)
    model.init_weights()
    ...
    if resuming:
        ...
        missing, unexpected = model.load_state_dict(
            model_data, strict=False, assign=use_assign
        )
        if missing:
            print0(
                f"New parameters initialized from scratch ({len(missing)} tensors): {', '.join(missing[:10])}{'...' if len(missing) > 10 else ''}"
            )
        if unexpected:
            print0(
                f"WARNING: unexpected keys in checkpoint ({len(unexpected)} tensors): {', '.join(unexpected[:10])}"
            )
    ...
    if args.fp8:
        if device_type != "cuda":
            ...
            num_fp8_layers = sum(
                1 for m in model.modules() if "Float8" in type(m).__name__
            )
            num_skipped = (
                sum(1 for m in model.modules() if isinstance(m, nn.Linear))
                - num_fp8_layers
            )
            print0(
    ...
    if args.use_scan and device_type == "xla":
        ...
        has_hybrid = getattr(model.config, "mamba_enabled", False) and any(
            getattr(b, "is_mamba", False)
            != getattr(model.transformer.h[0], "is_mamba", False)
            for b in model.transformer.h
        )
        if has_hybrid:
            print0(
                "Warning: --use_scan disabled for hybrid Mamba+Attention model "
            ...
            try:
                ...
                print0(
                    f"Enabled XLA scan_layers for faster compilation ({num_layers} layers -> 1 compiled block)"
                )
            except (ImportError, AttributeError):
                print0(
                    "Warning: --use_scan requires torch_xla >= 2.10 (scan_layers not found in this version), ignoring"
                )
    elif args.use_scan:
    ...
    assert (
        ...
        or args.target_param_data_ratio > 0
        or args.target_flops > 0
    )
    ...
    print0(
        f"Tokens : Params ratio: {args.total_batch_size * num_iterations / num_scaling_params:.2f}"
    )  # Chinchilla is ~20
    ...
    if spmd_mesh is not None:
        ...
        print0(
            f"SPMD dataloader batch size: {dataloader_B} (= {args.device_batch_size} per chip x {ddp_world_size} chips)"
        )
    ...
    if is_cpp_tokenizer:
        ...
        if cpp_markers == 0:
            ...
            print0(
                f"Checked {total_checked} samples: 0 contained #include, ::, or multiple semicolons"
            )
            print0("If training on non-C++ data, set NANOCHAT_CPP_TOKENIZER=0")
            print0("=" * 60)
        else:
            print0(
                f"Data check: {cpp_markers}/{total_checked} samples contain C++ markers"
            )
    ...
    mfu = 0.0  # initialized before loop; updated each training step
    # -----------------------------------------------------------------------------
    # Training loop
    while True:
        last_step = (
        ...
        if args.eval_every > 0 and (last_step or step % args.eval_every == 0):
            ...
            with disable_fp8(model), autocast_ctx():
                val_bpb = evaluate_bpb(
                    model, val_loader, eval_steps, token_bytes, synchronize=synchronize
                )
        ...
        if (
            ...
            try:
                ...
                print0(
                    f"Step {step:05d} | C++ compile: {results['cpp_compile_rate']:.1%}, pass: {results['cpp_pass_rate']:.1%}"
                )
                wandb_run.log(
                    {
                        "step": step,
                ...
                for prompt in prompts:
                    ...
                    with disable_fp8(orig_model), autocast_ctx():
                        sample, _ = engine.generate_batch(
                            tokens, num_samples=1, max_tokens=128, temperature=0
                        )
