...
# Model loading
parser.add_argument(
    "--model_tag", type=str, default=None, help="model tag to load from"
)
parser.add_argument(
...
parser.add_argument(
    "--init_lr_frac", type=float, default=1.0, help="initial LR as fraction of base LR"
)
# Evaluation
parser.add_argument(
    "--eval_every",
...
# Output
parser.add_argument(
    "--dry_run", action="store_true", help="log to wandb but skip checkpoints/report"
)
...
def mid_data_generator(split):
    ...
    needed_tokens = (
        args.device_batch_size * args.max_seq_len + 1
    ...
    while True:
        ...
        inputs = inputs_cpu.view(args.device_batch_size, args.max_seq_len).to(
            device=device, dtype=torch.int32, non_blocking=True
        )
        targets = targets_cpu.view(args.device_batch_size, args.max_seq_len).to(
